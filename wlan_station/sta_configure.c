...
static long ViLinkConnectAP()
{
	...
	if(lRetVal < 0)
	{
		UART_PRINT("lRetVal [%d]:Failed to establish connection w/ an AP \n\r", lRetVal);
	    return lRetVal;
	}
	UART_PRINT("WlanConnect... \n\r");

#if 0
	lRetVal = CheckLanConnection();
	if(lRetVal < 0)
	{
		UART_PRINT("lRetVal [%d]:Device couldn't ping the gateway \n\r", lRetVal);
	    return lRetVal;
	}
	UART_PRINT("CheckLanConnection ok... \n\r");
#endif

    return lRetVal;
}

long ViLinkStartStation()
{
    long lRetVal = -1;
    InitializeAppVariables();
	...
	if(lRetVal < 0)
	{
		...
		{
			UART_PRINT("lRetVal [%d]:Failed to configure the device in its default state\n\r", lRetVal);
		}
	    return lRetVal;
	}
	UART_PRINT("ConfigureSimpleLinkToDefaultState ok... \n\r");

	lRetVal = sl_Start(0, 0, 0);
	if (lRetVal < 0 || ROLE_STA != lRetVal)
	{
		UART_PRINT("lRetVal [%d]:Failed to start the device \n\r", lRetVal);
	    return lRetVal;
	}
	UART_PRINT("sl_Start ok... \n\r");

	lRetVal = ViLinkConnectAP();
	if (lRetVal < 0)
	{
		UART_PRINT("lRetVal [%d]:Failed to connect the ap \n\r", lRetVal);
		return lRetVal;
	}
...
long ViLinkStopStation()
{
    ...
    if(lRetVal < 0)
	{
		UART_PRINT("lRetVal [%d]:Device couldn't power off the network processor\n\r", lRetVal);
		return lRetVal;
	}
